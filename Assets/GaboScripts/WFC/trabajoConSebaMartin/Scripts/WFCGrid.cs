...
public class WFCGrid : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            ...
            {
                grid[i, j] = Instantiate<PossibilityTile>(possibilityTilePrefab, transform);
    ...
    void WFC()
    {
        ...
        while (!IsCollapsed() && count != 0)//!IsCollapsed())
        {
            ...
            TileType tileType = currPTile.GetLocalPossibleTileTypes()[UnityEngine.Random.Range(0, range)];
    ...
    (PossibilityTile, TileType) SetAndCollapseFirstRandom()
    {
        ...
        int posY = UnityEngine.Random.Range(0, height);
        PossibilityTile firstPTile = grid[posY, posX];

        int chosenTileTypeIndex = UnityEngine.Random.Range(0, firstPTile.GetLocalPossibleTileTypes().Count);
    ...
    bool DirectionalSinglePropagation(PossibilityTile pOrigin, WFCDirection direction)
    {
        ...
        switch (direction)
        {
            ...
            case WFCDirection.RIGHT:
                ...
                break;
            default:
                pObjective = Instantiate<PossibilityTile>(possibilityTilePrefab);
                break;
        }
